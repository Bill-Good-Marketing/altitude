...
function runCommand(cmd) {
  ...
  try {
    // Use 'pipe' or 'inherit' depending on whether you want to capture output.
...
function sleep(seconds) {
  ...
  try {
    ...
    // Fallback if sleep command is unavailable
  }
}
...
// 2) Ensure required schemas exist (createSchemas.sql)
...
// 5) Optionally clear existing data if CLEAR_DB=true
...
// 6) (Optional) Additional ensures – if needed, these can be no-ops
...
// 7) Update timezone data if needed
...
// 9) Seed the database with import-playground.ts
