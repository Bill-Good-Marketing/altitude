// app/(auth)/token-refresh/page.tsx (or whichever file defines your route)
...
export default async function RefreshTokenPage({ searchParams }: { searchParams?: Promise<{ redirect?: string }> }) {
    ...
    if (user === 'refresh') {
        ...
        return (
            <SessionWrapper>
                <TokenRefresh actuallyRefresh={false} redirectUri={_redirect ?? '/'} />
            </SessionWrapper>
        );
    }
}
